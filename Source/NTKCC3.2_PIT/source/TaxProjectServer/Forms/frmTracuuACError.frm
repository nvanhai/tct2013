...
Begin VB.Form frmTraCuuACError 
   ...
   Begin FPUSpreadADO.fpSpread fpsKetQua 
      ...
      SpreadDesigner  =   "frmTracuuACError.frx":0000
   End
   ...
   End
End
Attribute VB_Name = "frmTraCuuACError"
...
Private Sub btnTraCuu_Click(Index As Integer)
    ...
    strSQL = GetAttribute(xmlSQL.childNodes(1), "SqlTraCuuAcError")
        ...
        Do While Not rsReturn.EOF
            ...
            For lIndex = 1 To rsReturn.Fields.Count
                If Not (IsNull(rsReturn.Fields(lIndex - 1).Value)) Then
                    ...
                    Else
                        fpsKetQua.SetText lCol, lRow, TAX_Utilities_Svr_New.Convert(Trim(rsReturn.Fields(lIndex - 1).Value), TCVN, UNICODE)
                    End If
                    lCol = lCol + 1
...
Private Sub Form_Load()
    SetControlCaption Me, "frmTraCuuACError"
    FormatGrid
    SetupData
    'lerror = False
    With fpsLoaiTK
...
Private Sub Form_Resize()
    ...
    frmTraCuuACError.Top = (frmSystem.Height - frmTraCuuACError.Height) / 4
    frmTraCuuACError.Left = (frmSystem.Width - frmTraCuuACError.Width) / 2
End Sub
