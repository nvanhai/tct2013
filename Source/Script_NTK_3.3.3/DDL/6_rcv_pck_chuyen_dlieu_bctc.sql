CREATE OR REPLACE PACKAGE BODY QLT_NTK.rcv_pck_chuyen_dlieu_bctc IS
    ...
    v_so_hs_goc varchar2(100);
      BEGIN
        For rec in (select hthuc_nop,id,itkhai_id,tin,ten_dtnt,dia_chi,ngay_cap_nhat,nguoi_cap_nhat,ngay_nop,phong_xly,kkbs,Kykk_Tu_Ngay,loai_tkhai from rcv_tkhai_hdr hdr where rownum <=500 and loai_tkhai in ('15_BCTC','15_BCTC10','48_BCTC','48_BCTC13','95_BCTC','16_BCTC') and (da_nhan is null or da_nhan='N') order by tin,kkbs,ngay_nop)
          loop
            begin
              ...
              insert into rcv_tkhai_hdr_bctc_log (ID,TIN,LOAI_TKHAI,NGAY_NOP,KYKK_TU_NGAY,NGAY_CAP_NHAT,NGUOI_CAP_NHAT,CO_LOI_DDANH,SO_TT_TK,PHONG_XLY,KKBS,DA_NHAN)
              select ID,TIN,LOAI_TKHAI,NGAY_NOP,KYKK_TU_NGAY,NGAY_CAP_NHAT,NGUOI_CAP_NHAT,CO_LOI_DDANH,SO_TT_TK,v_phongxl,KKBS,'N' from rcv_tkhai_hdr
              where id=rec.id;
              ...
              EXCEPTION
                WHEN OTHERS THEN
                  begin
                    ...
                    insert into rcv_chuyen_dlieu_err_log(hdr_id,loai_tkhai,ma_loi,thong_bao_loi,ngay_thuc_hien)
                    values(rec.id,rec.loai_tkhai,errc,errm,sysdate);
                    --Update cho IHTKK
                    if(rec.itkhai_id is not null) then
