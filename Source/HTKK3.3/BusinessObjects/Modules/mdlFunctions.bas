...
Public Sub UpdateCell(fps As fpSpread, ByVal pCol As Long, ByVal pRow As Long, ByVal pValue As String)
    ...
    Set xmlNodeCell = TAX_Utilities_v2.Data(mCurrentSheet - 1).nodeFromID(GetCellID(fps, pCol, pRow))
...
Public Sub UpdateCell_sheet(fps As fpSpread, ByVal pSheet As Long, ByVal pCol As Long, ByVal pRow As Long, ByVal pValue As String)
    ...
    Set xmlNodeCell = TAX_Utilities_v2.Data(pSheet - 1).nodeFromID(GetCellID(fps, pCol, pRow))
    If Not xmlNodeCell Is Nothing Then
        SetAttribute xmlNodeCell, "Value", pValue
    End If
...
Public Sub UpdateKHBSCell(fps As fpSpread, ByVal pCol As Long, ByVal pRow As Long, ByVal pValue As String)
    Dim xmlNodeCell As MSXML.IXMLDOMNode
    GetCellSpan fps, pCol, pRow
    Set xmlNodeCell = TAX_Utilities_v2.Data(0).nodeFromID(GetCellID(fps, pCol, pRow))
...
Public Sub UpdateLastKHBSCell(fps As fpSpread, ByVal pCol As Long, ByVal pRow As Long, ByVal pValue As String)
    ...
    Set xmlNodeCellData0 = TAX_Utilities_v2.Data(0).nodeFromID(GetCellID(fps, pCol, pRow))
    
    If Not xmlNodeCellData0 Is Nothing Then
        SetAttribute xmlNodeCellData0, "Value", pValue
    End If
    
    Set xmlNodeCell = TAX_Utilities_v2.DataKHBS.nodeFromID(GetCellID(fps, pCol, pRow))
    loaiKHBS11 = GetAttribute(TAX_Utilities_v2.Data(TAX_Utilities_v2.NodeValidity.childNodes.length - 1).childNodes(2).firstChild, "loaiKHBS")
    If Not xmlNodeCell Is Nothing Then
...
'    If tfView Then fps.value = Val(fps.value) / 1000
    ...
    If tfView Then
        If GetAttribute(TAX_Utilities_v2.NodeMenu, "ID") = "12" Then
            Set xmlNode = TAX_Utilities_v2.Data(0).nodeFromID("K_47")  'J_42: thue suat uu dai
            tempValue = GetAttribute(xmlNode, "Value")
            fps.value = tempValue
        ElseIf GetAttribute(TAX_Utilities_v2.NodeMenu, "ID") = "11" Then
            Set xmlNode = TAX_Utilities_v2.Data(0).nodeFromID("K_34")  'J_42: thue suat uu dai
            tempValue = GetAttribute(xmlNode, "Value")
            fps.value = tempValue
        End If
    End If
...
Public Function MYCompare(StrMY As String) As Boolean
    ...
    MY2 = DateSerial(TAX_Utilities_v2.Year, TAX_Utilities_v2.Month, 1)
    If MY1 <= MY2 Then
        MYCompare = True
    Else
        MYCompare = False
...
Public Function GetCatalogueFileName(ByRef blnFollowCheck As Boolean, Optional lSheet As Long = 1) As String
    ...
    Set xmlCatalogeValidNode = GetValidityNode("100_7", TAX_Utilities_v2.Month, TAX_Utilities_v2.ThreeMonths, TAX_Utilities_v2.Year)
    ...
    End If
    'Get catalogue ID
    strCatalogueID = GetAttribute(TAX_Utilities_v2.NodeValidity, "CatalogueID")
    'Get catalogue pattern name
    strCatalogueName = GetCatalogueName(xmlCatalogeValidNode, strCatalogueID)
    
    'Get catalogue file name
    strReturn = GetAbsolutePath(TAX_Utilities_v2.DataFolder & _
    ...
    strReturn = GetAbsolutePath(GetAttribute(TAX_Utilities_v2.NodeValidity.childNodes(lSheet - 1), "TemplateFolder") & _
                strCatalogueName & ".xml")
...
Public Function GetCatalogueTemplateFile(Optional lSheet As Long = 1) As String
    ...
    strReturn = GetAttribute(TAX_Utilities_v2.NodeValidity.childNodes(lSheet - 1), "TemplateFolder") & _
        strCatalogueName & ".xml"
