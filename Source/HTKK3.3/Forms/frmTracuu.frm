...
Private Sub btnMo_Click()
    ...
    With fpSKetQua
        ...
        If Left(varId, 4) = "KHBS" Then
            varId = Right(varId, 2)
            strKHBS = "frmKHBS_BS"
            TAX_Utilities_v2.DateKHBS = varDateKHBS
        End If
        ...
        TAX_Utilities_v2.NodeMenu = getNode(CStr(varId))

        ' 12110211 xu ly to khai BS
        If strKHBS = "TKBS" Then

            For i = 1 To TAX_Utilities_v2.NodeMenu.childNodes(0).childNodes.length - 1

                If i = TAX_Utilities_v2.NodeMenu.childNodes(0).childNodes.length - 1 Then
                    SetAttribute TAX_Utilities_v2.NodeMenu.childNodes(0).childNodes(i), "Active", "1"
                Else
                    SetAttribute TAX_Utilities_v2.NodeMenu.childNodes(0).childNodes(i), "Active", "0"
                End If
        ...
        If LoaiTk = KIEU_KY_THANG Then
            TAX_Utilities_v2.month = Mid$(CStr(varPeriod), 1, 2)
            TAX_Utilities_v2.Year = Mid$(CStr(varPeriod), 4, 4)

            If tkThangQuy = "1" Then
                strQuy = "TK_THANG"
                TAX_Utilities_v2.ThreeMonths = CInt(Mid$(CStr(varPeriod), 1, 2))
            End If
        ...
        ElseIf LoaiTk = KIEU_KY_QUY Then
            ...
            strQuy = "TK_QUY"
            If tkThangQuy = "1" Then
                TAX_Utilities_v2.month = CInt(Mid$(CStr(varPeriod), 1, 2))
            End If
        ElseIf LoaiTk = KIEU_KY_NGAY_PS Then
            TAX_Utilities_v2.Day = Mid$(CStr(varPeriod), 1, 2)
            TAX_Utilities_v2.month = Mid$(CStr(varPeriod), 4, 2)
            TAX_Utilities_v2.Year = Right(CStr(varPeriod), 4)
        ...
        ElseIf LoaiTk = KIEU_KY_TU_NGAY_DEN_NGAY Then
            TAX_Utilities_v2.Year = CStr(varPeriod)
            TAX_Utilities_v2.FirstDay = CStr(varFirstDay)
            TAX_Utilities_v2.LastDay = CStr(varLastDay)
        ElseIf LoaiTk = "K" Then
            TAX_Utilities_v2.ThreeMonths = CInt(Mid$(CStr(varPeriod), 1, 2))
            TAX_Utilities_v2.Year = Mid$(CStr(varPeriod), 4, 4)
            TAX_Utilities_v2.FirstDay = getCellValue(Left$(varFileName, InStr(varFileName, ",") - 1), "D_17")
            TAX_Utilities_v2.LastDay = getCellValue(Left$(varFileName, InStr(varFileName, ",") - 1), "E_17")
        ...
        ElseIf LoaiTk = KIEU_KY_THANG_NAM Then
            TAX_Utilities_v2.month = Mid$(CStr(varPeriod), 1, 2)
            TAX_Utilities_v2.Year = Mid$(CStr(varPeriod), 4, 4)
            TAX_Utilities_v2.FirstDay = varFirstDay
            TAX_Utilities_v2.LastDay = varLastDay
            strQuy = "TK_TU_THANG"
            strLoaiTKThang_PS = "TK_TU_THANG"
            TAX_Utilities_v2.ThreeMonths = GetQuyHienTai(iNgayTaiChinh, iThangTaiChinh).q
...
'                If fso.FileExists(GetAbsolutePath(TAX_Utilities_v2.DataFolder & strDataFile & ".xml")) Then
'                    fso.DeleteFile GetAbsolutePath(TAX_Utilities_v2.DataFolder & strDataFile & ".xml"), True
'                    .DeleteRows i + 1, 1
'                End If
            End If
        Next
...
Sub SetupData()
    ...
    With fpsLoaiTK
        ...
        xmlDocument.Load TAX_Utilities_v2.GetAbsolutePath("Map.xml")
...
Private Function GetTaxValue(ByVal strDataFileName As String, ByVal strId As String, ByVal KHBS As Boolean) As String
    ...
    xmlDom.Load TAX_Utilities_v2.DataFolder & strDataFileName & ".xml"
    If KHBS = False Then
...
Private Function getKHBSDate(DataFile As String) As String
    ...
    For Each KHBSfile In DataFiles
        ...
        If InStr(KHBSfile, "KHBS") > 0 Then
            If fso.FileExists(GetAbsolutePath(TAX_Utilities_v2.DataFolder & KHBSfile & ".xml")) Then
                xmlDoc.Load GetAbsolutePath(TAX_Utilities_v2.DataFolder & KHBSfile & ".xml")
...
Private Function getCellValue(DataFile As String, cellID As String) As String
    ...
    If fso.FileExists(GetAbsolutePath(TAX_Utilities_v2.DataFolder & DataFile & ".xml")) Then
        xmlDoc.Load GetAbsolutePath(TAX_Utilities_v2.DataFolder & DataFile & ".xml")
...
Sub ResetAllProperty()
    ...
    TAX_Utilities_v2.DateKHBS = ""
    TAX_Utilities_v2.Day = ""
