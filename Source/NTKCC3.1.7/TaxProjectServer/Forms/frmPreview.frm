...
On Error GoTo ErrHandle
    ...
    'If fpspReport.PageCurrent < frmReportData.fpsReport.PrintPageCount Or frmReportData.fpsReport.Sheet < frmReportData.fpsReport.SheetCount - 1 Then
        ...
        Else ' Move next Sheet
                ...
                intLocalCurrPage = 1
'            Loop Until GetAttribute(TAX_Utilities_Svr_New.NodeValidity. _
...
'            If GetAttribute(TAX_Utilities_Svr_New.NodeValidity. _
...
'     If GetAttribute(TAX_Utilities_Svr_New.NodeValidity.childNodes(1), "Caption") = "04-1/TNCN" Then
...
Private Sub fpSpread1_TextTipFetch(ByVal Col As Long, ByVal Row As Long, MultiLine As FPUSpreadADO.TextTipFetchMultilineConstants, TipWidth As Long, TipText As String, ShowTip As Boolean)
    ...
    Select Case Col
        Case 2 'cmdNext
            Set xmlNode = GetNodeByName(TAX_Utilities_Svr_New.NodeCaption.selectSingleNode(Me.Name).childNodes, "cmdNext")
            If Not xmlNode Is Nothing Then
                'TipWidth = 800
                TipText = GetAttribute(xmlNode, "Caption") & " [Ctrl+N] "
            End If
        Case 4 'cmdPrevious
            Set xmlNode = GetNodeByName(TAX_Utilities_Svr_New.NodeCaption.selectSingleNode(Me.Name).childNodes, "cmdPrevious")
            If Not xmlNode Is Nothing Then
                'TipWidth = 900
                TipText = GetAttribute(xmlNode, "Caption") & " [Ctrl+P] "
            End If
        Case 6 'cmdZoom
            Set xmlNode = GetNodeByName(TAX_Utilities_Svr_New.NodeCaption.selectSingleNode(Me.Name).childNodes, "cmdZoom")
            If Not xmlNode Is Nothing Then
                'TipWidth = 1440
                TipText = GetAttribute(xmlNode, "Caption") & " [Ctrl+Z] "
            End If
        Case 8 'cmdPrint
            Set xmlNode = GetNodeByName(TAX_Utilities_Svr_New.NodeCaption.selectSingleNode(Me.Name).childNodes, "cmdPrint")
            If Not xmlNode Is Nothing Then
                'TipWidth = 620
                TipText = GetAttribute(xmlNode, "Caption") & " [Ctrl+I] "
            End If
        Case 16 'cmdClose
            Set xmlNode = GetNodeByName(TAX_Utilities_Svr_New.NodeCaption.selectSingleNode(Me.Name).childNodes, "cmdClose")
            If Not xmlNode Is Nothing Then
                'TipWidth = 620
