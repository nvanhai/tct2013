        ...
        FROM
            (
                SELECT
                    ...
                    (DECODE(gdien.cot_04, tkd.ky_hieu, tkd.gia_tri, NULL)) So_BLP,
                    DECODE(gdien.cot_05, tkd.ky_hieu, tkd.gia_tri, NULL)       Ngay_BLP,
                    (DECODE(gdien.cot_06, tkd.ky_hieu, tkd.gia_tri, NULL)) Ten_BLP,
                    (DECODE(gdien.cot_07, tkd.ky_hieu, tkd.gia_tri, NULL)) Mau_so,
                    (DECODE(gdien.cot_08, tkd.ky_hieu, tkd.gia_tri, NULL)) Ky_hieu_BLP,
...
--01_TBAC            
CREATE OR REPLACE VIEW QLT_NTK.RCV_V_01_TBAC_BLP_TEST AS
SELECT
    hdr_id ,
    row_id so_tt ,
    ten_BLP ,
        ...
        FROM
            (
                SELECT
                    ...
                    (DECODE(gdien.cot_10, tkd.ky_hieu, tkd.gia_tri, NULL)) Ten_DN_in,
...
--03_TBAC            
CREATE OR REPLACE VIEW
    QLT_NTK.RCV_V_03_TBAC_BLP_TEST
    (
        HDR_ID,
        SO_TT,
        TEN_BLP,
...
FROM
    (
        ...
        GROUP BY
            ...
            dtl.row_id ) ;
--BC21_AC            
CREATE OR REPLACE VIEW QLT_NTK.RCV_V_BC21_AC_BLP_TEST AS
SELECT
    hdr_id ,
    row_id so_tt ,
    ten_BLP ,
        ...
        FROM
            (
                SELECT
                    ...
                    gdien.id          id,
                    gdien.so_tt,
                    (DECODE(gdien.cot_02, tkd.ky_hieu, tkd.gia_tri, NULL)) ten_BLP,
                    (DECODE(gdien.cot_01, tkd.ky_hieu, tkd.gia_tri, NULL)) Mau_so,
                    (DECODE(gdien.cot_03, tkd.ky_hieu, tkd.gia_tri, NULL)) Ky_hieu_BLP,
                    DECODE(gdien.cot_04, tkd.ky_hieu, tkd.gia_tri, NULL)       Tu_so,
                    DECODE(gdien.cot_05, tkd.ky_hieu, tkd.gia_tri, NULL)       Den_so,
                    DECODE(gdien.cot_06, tkd.ky_hieu, tkd.gia_tri, NULL)       So_luong,
                    (DECODE(gdien.cot_07, tkd.ky_hieu, tkd.gia_tri, NULL)) Lien_BLP,
                    (DECODE(gdien.cot_08, tkd.ky_hieu, tkd.gia_tri, NULL)) Ghi_chu,
                    tkd.id                                                     dtl_id
                FROM
                    QLT_NTK.rcv_bcao_dtl_ac tkd,
                    QLT_NTK.rcv_gdien_tkhai gdien,
...
--BC26_AC
create or replace view QLT_NTK.rcv_v_bc26_ac_blp_TEST as
select
    hdr_id ,
    row_id so_tt ,
    ten_BLP ,
...
CREATE OR REPLACE VIEW QLT_NTK.RCV_V_BC26_HDR_BLP_TEST
...
SELECT  id,
  ...
  FROM QLT_NTK.rcv_bcao_hdr_ac
    Where Loai_Bc = 'BC26_AC_BLP'
        And Da_Nhan Is Null;
--VIEW HDR 
CREATE OR REPLACE VIEW QLT_NTK.RCV_V_HDR_BLP_TEST
...
SELECT ID, tin, loai_bc, ngay_nop, kybc_tu_ngay, kybc_den_ngay,
          ngay_cap_nhat,  (nguoi_cap_nhat), so_tt_tk, da_nhan, phong_xly,
          phong_qly, co_bang_ke, hthuc_nop, itkhai_id,  (ten_dv_cq), tin_dv_cq,
          ngay_bc,  (nguoi_dai_dien),  (ten_cq_tiep_nhan),  (ly_do_mat),
          ngay_mat_huy,  (phuong_phap_huy), dung_dn_cq,  (ghi_chu), ma_cqt,
          loai_bc26,  (nguoi_lap_bieu), quy_bc, ngay_tb_ph
     FROM QLT_NTK.rcv_bcao_hdr_ac
