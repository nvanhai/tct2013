CREATE OR REPLACE PACKAGE qlt_ntk.rcv_pck_chuyen_dlieu_bctc is
   Procedure Prc_load_ntk_2_bctc_log;
   FUNCTION Fnc_Get_So_Hoso(p_Date IN VARCHAR2, p_Ma IN VARCHAR2) RETURN VARCHAR2;
   FUNCTION Fnc_So_Hoso_exist(p_tin IN VARCHAR2, p_kykk_tu_ngay IN DATE) RETURN VARCHAR2;
End rcv_pck_chuyen_dlieu_bctc
/
CREATE OR REPLACE PACKAGE BODY qlt_ntk.rcv_pck_chuyen_dlieu_bctc IS
    ...
    v_so_hs_goc varchar2(100);
      BEGIN
        For rec in (select id,itkhai_id,tin,ten_dtnt,dia_chi,ngay_cap_nhat,nguoi_cap_nhat,ngay_nop,phong_xly,kkbs,Kykk_Tu_Ngay from rcv_tkhai_hdr hdr where rownum <=500 and loai_tkhai in ('15_BCTC','15_BCTC10','48_BCTC','48_BCTC13','95_BCTC','16_BCTC') and (da_nhan is null or da_nhan='N') order by tin,kkbs,ngay_nop)
          loop
            begin
