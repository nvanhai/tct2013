...
Public Function DisplayMessage(pMsgID As String, Optional pMsgStyle As MsgBoxStyle, Optional pIcon As MsgBoxIcon, Optional pTitle As String, Optional msType As Byte) As MsgBoxResult
    ...
    Select Case pMsgStyle
        ...
        Case msYesNoCancel
            If msType = 0 Then
                ...
                ButtonCount = 3
            ElseIf msType = 1 Then ' Truong hop nay de hien thi ghi va
                clsMess.SetControlCaption Me, "frmYesNoCancelKHBS"
                SetButtonValue mrYes, mrNo, mrCancel
                ButtonCount = 3
            End If
    End Select
...
Private Sub SetMessage(pMsgID As String)
    ...
    Dim vMessages() As String
    Dim iFlagGetTTTK As Boolean
    
    lblMessage.Caption = ""
    If InStr(1, Trim(pMsgID), "#") = 1 Then
        pMsgID = Mid(pMsgID, 2)
    ElseIf InStr(1, Trim(pMsgID), "#") > 1 Then
        vMessages = Split(pMsgID, "#")
        pMsgID = vMessages(1)
        iFlagGetTTTK = True
    End If
    ...
    For Each xmlNode In xmlNodeListMessage
        If xmlNode.Attributes.getNamedItem("ID").nodeValue = pMsgID Then
            If iFlagGetTTTK Then
                lblMessage.Caption = vMessages(0) & "  " & xmlNode.Attributes.getNamedItem("Msg").nodeValue
            Else
