...
'    thongTinTK = thongTinTK + "   " + Right(strTaxReportInfo, 6)
    With fpSpread1
        ...
        If Trim(varTemp) <> vbNullString Then
            thongTinTK = thongTinTK + TAX_Utilities_Srv_New.Convert(" Loπi TK : TKCT ", TCVN, UNICODE)
        Else 'neu la to bo sung
            ...
            .GetText lCol, lRow, varTemp
            thongTinTK = thongTinTK + TAX_Utilities_Srv_New.Convert(" Loπi TK: BS  ", TCVN, UNICODE)
        End If
        'set so lan nop tk
        thongTinTK = thongTinTK + TAX_Utilities_Srv_New.Convert(" L«n nÈp : ", TCVN, UNICODE) + lanNopTKIHTKK
        thongTinTK = thongTinTK + TAX_Utilities_Srv_New.Convert(" TÍ khai : ", TCVN, UNICODE) + changeMaLoaiToKhai(Trim(TAX_Utilities_Srv_New.NodeMenu.Attributes.getNamedItem("ID").nodeValue))
